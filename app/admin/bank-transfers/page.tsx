  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (!updateData || updateData.length === 0) {
        throw new Error(
          "Update blocked — check RLS policy on bank_transfer_requests"
        )
      }
      ...
      if (action === "approve") {
        ...
        if (!planData || planData.length === 0) {
          throw new Error("Plan update blocked — check RLS policy on profiles")
        }
      }

      // Immediately update local state so stats cards + table reflect change
      setRequests((prev) =>
        prev.map((r) =>
