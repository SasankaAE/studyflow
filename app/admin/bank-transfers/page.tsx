  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (!updateData || updateData.length === 0) {
        throw new Error(
          "RLS blocked bank_transfer_requests update — check admin policy"
        )
      }
      ...
      if (action === "approve") {
        ...
        if (!planData || planData.length === 0) {
          throw new Error(
            "RLS blocked profiles update — check admin policy on profiles table"
          )
        }
      }
          ...
          {pendingDecision && (
            <div className="space-y-3 text-sm">
              <p>
                ...
                {pendingDecision.request.profiles?.email}
              </p>
              <p>
                <span className="text-muted-foreground">Reference:</span>{" "}
                <span className="font-mono">
                  {pendingDecision.request.reference_number}
                </span>
              </p>
              <p>
                <span className="text-muted-foreground">Plan change:</span>{" "}
                current → <strong>{pendingDecision.request.target_plan}</strong>
              </p>
              {pendingDecision.action === "approve" && (
                <div className="rounded border border-green-200 bg-green-50 p-3 text-xs text-green-800 dark:border-green-800 dark:bg-green-950 dark:text-green-300">
                  ...
                  <strong>{pendingDecision.request.target_plan}</strong>.
                </div>
              )}
              <div>
