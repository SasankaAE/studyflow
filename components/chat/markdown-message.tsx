      ...
      components={{
        p({ children, ...props }) {
          return (
            <p
              {...props}
              className="mb-2 break-words whitespace-pre-wrap last:mb-0"
            >
              {children}
            </p>
          )
        ...
        pre({ children, ...props }) {
          return (
            <pre
              {...props}
              className="my-2 overflow-x-auto rounded-lg bg-black/30 p-3 text-xs leading-relaxed sm:text-sm"
            >
              {children}
            </pre>
          )
        ...
        code({ children, className, ...props }) {
          ...
          return isInline ? (
            <code
              {...props}
              className="break-words rounded bg-black/20 px-1 py-0.5 font-mono text-xs"
            >
              {children}
            </code>
          ) : (
        ...
        td({ children, ...props }) {
          return (
            <td
              {...props}
              className="border border-border px-2 py-1 break-words"
            >
              {children}
            </td>
          )
